...
int main() {
	...
	double parCpuTime = ((double) 10e3 * (end - start)) / CLOCKS_PER_SEC / EXECUTIONS;
	...
	bool equal2 = checkEquality(cs.data(), cp_cpu.data(), ROWS_C, COLS_C);

	/**
	 * Print results.
	 * */

	std::cout << "Status: " << (equal1 && equal2 ? "SUCCESS!" : "FAILED!") << std::endl;
	std::cout << "Results: \n\tA[0] = " << a[0] << "\n\tB[0] = " << b[0] << "\n\tC[0] = " << cp[0] << std::endl;
	std::cout << "Mean execution time: \n\tSequential: " << seqTime << " ms;\n\tParallel: " << parTime << " ms;\n\tParallel_CPU: " << parCpuTime << " ms." << std::endl;
